...
5. Inspect the bottom-left transparent HUDs by calling `window.game.inventory.toggle()` and `window.game.player.inventory.getRenderState()` from the console to ensure container overlays reuse the same grid rendering logic.
